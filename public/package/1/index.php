...
<div class = "text-center container-fluid">
	<div class = "row page-header checkout_form">
		<div class = "container">
...
<div class = "container checkout_form">
	<div class = "container-fluid text-center title">
		<h2><span class = "username"></span> Upgrade today!</h2>
		<h3> You're just <span>60 seconds</span> away from your new Premium Account</h3>
	</div>
	<?php
	/*	if (isset($_POST['submit']) == "submit" && isset($offer)) {
			if ($offer->dating->purchase_status == false || $offer->live->purchase_status == false ||
				$offer->video->purchase_status == false) {
				echo "<p align='center' style='color:#ffffff;background-color:red;'>" .
					"Oops, something went wrong. Please verify your information or try another card." . "</p>";
				if (DEBUG_PRINT) {
	...
	?>
	<div class = "row content_wrap d-flex flex-row">
		<div class = "col-12 col-md-5 column_one details text-center d-flex">
			<div class = "col-md-12 column justify-content-center">
				<h3>1 Day FREE Membership</h3>
				<h2>$0.00</h2>
				<p class = "gray">There will be no charge today!</p>
				<ul>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Send & Reply to messages</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Priority ranking in search results</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">View private photos</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Priority support handling</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Include contact info in messages</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Visit unlimited profiles</p>
						</div>
					</li>
					<li>
						<div class = "float-left check_desc">
							<p class = "float-left">Premium indicator on profile</p>
						</div>
					</li>
					<li class = "vip">
						<!--<div class="col-2 float-left check_img">
                            <a href="#"><img class="float-right" src="<? /*=IMAGES*/ ?>/checkmark.jpg" alt=""></a>
                        </div>-->
						<div class = "float-left vip_check">
							<input type = "checkbox" name = "vip" id = "vip" checked>
							<label for = "vip">VIP Offers</label>
						</div>
						<!--<div class="col-10 float-left check_desc">
							<p class="float-left">VIP Offers</p>
						</div>-->
					</li>
				</ul>
			</div>
		...
		<div class = "col-12 col-md-7 col-lg-7 column_two d-flex">
			<div class = "col-md-12 column">
				<form class = "form-horizontal container-fluid" method = "post"
					  action = "<?php echo $_SERVER['REDIRECT_URL']; ?>">
					<fieldset>
						<!-- Form Name -->
						...
						<div class = "row">
							<div class = "form-group col-md-6">
								...
								<input id = "cardNo" name = "cardNo" type = "text" placeholder = ""
									   class = "form-control input-md" required = ""
									   value = "<?php
									   if (isset($_SESSION['user']->cardNo)) {
										   echo $_SESSION['user']->cardNo;
									   } ?>">
							</div>
							<div class = "form-group col-md-3">
								<label class = "control-label" for = "expireMonth">Month:</label>
								<select id = "expireMonth" name = "expireMonth" class = "form-control" required = "">
							...
							<div class = "form-group col-md-3">
								...
								<select id = "expireYear" name = "expireYear" class = "form-control year" required = "">
						...
						<div class = "row">
							<div class = "form-group col-md-6">
								...
								<input id = "cvv2" name = "cvv2" type = "text" placeholder = ""
									   class = "form-control input-md"
									   required = "" value = "<?php if (isset($_POST['cvv2'])) {
									echo $_POST['cvv2'];
								} ?>">
							</div>
							<!--<div class="form-group col-md-6">
						...
						<div class = "row" hidden>
							<label class = "custom-control custom-checkbox col-md-12">
								<div class = "">
									...
									<label for = "vip_hidden">VIP Offers</label>
								</div>
							</label>
						...
						<div class = "row submit">
							...
							<p>Your trial to <?php echo $_SERVER['SERVER_NAME'] ?> will renew for twenty nine ninety
							   five a month after twenty
							   four hours and monthly until cancelled.</p>
						</div>
		...
		<div class = "row">
			<div class = "col-12">
				<p>Vip offer will include a token purchase to
				   live.<?php echo DOMAIN ?> of thirty
				   dollars and a fourteen day
				   free trial of videos.<?php echo DOMAIN ?> an exclusive library
				   of
				   adult content
	...
	<div class = "modal-dialog" role = "document">
		<div class = "modal-content">
			...
			<div class = "modal-body">
				...
				<p><?= DOMAIN ?> (&ldquo;we,&rdquo; &ldquo;us&rdquo; or
								 &ldquo;<?= DOMAIN ?>&rdquo;) offers&nbsp;our site&nbsp;(&ldquo;Site&rdquo;) to
				...
				<p>These Terms, as well as any and all Agreements referenced and incorporated
				   herein, shall form a binding contract between You and <?= DOMAIN ?>. By
				...
				<p>You agree that the information You provide during registration, including but not
				   ...
				   individual, <?= DOMAIN ?>, or other entity to use Your Servicer Account.</p>
				<h3>Subscription, Credits and Fees</h3>
				<p>You grant permission to <?= DOMAIN ?> to make charges to Your credit card as
				...
				<p>You agree that You will use the Service in compliance with all applicable local,
				   ...
				   to state and federal penalties and other legal consequences. <?= DOMAIN ?>
				...
				<p>The source of the Content may vary between Servicers and <?= DOMAIN ?>. To the
				   extent that Content originates from <?= DOMAIN ?>, such Content is the sole
				   property of <?= DOMAIN ?>. You are only permitted to use Content
				   from <?= DOMAIN ?> as expressly permitted by us. Except for a single copy made
				...
				<p>To the extent that the Services provide Servicers an opportunity to post, store
				   ...
				   grant <?= DOMAIN ?> and the Servicers of the Site an irrevocable, perpetual,
				...
				<p><?= DOMAIN ?> respects copyright law and expects Servicers to do the same.</p>
				<p>If You believe that Your work has been copied in a way that constitutes copyright
				   infringement, please provide <?= DOMAIN ?>&rsquo;s Agent for Notice of claims
				...
				<p><?= DOMAIN ?> reserves the right, in its sole discretion, to determine whether
				...
				<p>By registering with Site, You understand that we may send You communications or
				   ...
				   regarding <?= DOMAIN ?> or other products and services via electronic mail. We
				...
				<p>The Service is made available to You for Your personal use only. You must provide
				   ...
				   the prior written permission of <?= DOMAIN ?>. All rights reserved. If You
				...
				<p>You represent and warrant to <?= DOMAIN ?> that Your use of the Service
				   ...
				   to <?= DOMAIN ?> (and which are relied upon by <?= DOMAIN ?>) as a material
				...
				<p><?= DOMAIN ?> offers the&nbsp;Services to You for the exclusive purpose of Your
								 personal and private recreation, entertainment, and enjoyment (the &ldquo;Service&rdquo;).
								 Specifically, as a Service, <span class = "text-uppercase"><?= DOMAIN ?></span>
							...
							class = "text-uppercase"><?= DOMAIN ?></span> makes no representations
				...
				<p><?= DOMAIN ?> also offers its subscribers access to Quality Control Agents
				...
				<p><?= DOMAIN ?> reserves the right in its sole discretion to refuse, suspend, or
								 ...
								 remaining for that month, at the sole discretion of <?= DOMAIN ?>. For more
				...
				<p>As a condition to using the Service, You agree to the terms of
				   this <?= DOMAIN ?> Privacy Policy as it may be updated from time to time. We
				   understand that privacy is important to You. You do, however, agree
				   that <?= DOMAIN ?> may monitor, edit or disclose Your Personal Information,
				...
				<p>You agree that <?= DOMAIN ?> may freely, for any sound business purpose,
				   ...
				   of <?= DOMAIN ?> and all such information, including that obtained from You,
				   may be used for any sound business purpose. In the routine course of its
				   business, <?= DOMAIN ?> will verify Your credit card information and email
				   ...
				   provided to <?= DOMAIN ?> may be read, stored, and used by its personnel. As a
				   visitor to&nbsp;Site if You provided Your email address in a registration
				   membership process or posted it on Site, You thereby authorize <?= DOMAIN ?>
				...
				<p>Personal Information collected by <?= DOMAIN ?> may be stored and processed in
				   the United States or any other country in which <?= DOMAIN ?> or its agents
				   maintain facilities. By using the Service, You consent to any such transfer of
				   information outside of Your country.</span></p>
				<h3>Warranty</h3>
				<p><?= DOMAIN ?> does not represent that the Service is reliable, accurate,
								 complete, or otherwise valid. <?= DOMAIN ?> cannot guarantee, and assumes no
								 obligation to verify the accuracy of any information submitted by its users, nor
								 does <?= DOMAIN ?> assume any obligation to monitor the activities of its
								 users. No information, whether written or oral, obtained by you
								 from <?= DOMAIN ?> shall create any warranty not expressly stated in this
								 Agreement. THE SERVICE IS PROVIDED &quot;AS IS&quot; WITH NO WARRANTY OF ANY
								 KIND AND YOU USE THE SERVICE AT YOUR OWN RISK. <span
							class = "text-uppercase"><?= DOMAIN ?></span> EXPRESSLY DISCLAIMS ANY
								 ...
								 NON-INFRINGEMENT.</p>
				<h3>Damages</h3>
				<p>UNDER NO CIRCUMSTANCES WILL <span class = "text-uppercase"><?= DOMAIN ?></span>
				   ...
				   NO CIRCUMSTANCES SHALL <span class = "text-uppercase"><?= DOMAIN ?></span> BE
				   LIABLE TO YOU FOR ANY AMOUNT FOR SERVICES RENDERED PURSUANT TO THIS AGREEMENT.
				</p>
				<h3>Indemnification</h3>
				<p>You agree to hold harmless and indemnify <?= DOMAIN ?>, and its employees,
				...
				<h3>To the maximum extent permitted by applicable law, You hereby release, and waive
					all claims against <?= DOMAIN ?> and its employees and agents from any and all
				...
				<h3>Exercise</h3>
				<p>The failure of <?= DOMAIN ?> to exercise or enforce any right or provision of
				...
				<p>This Agreement constitutes the entire agreement between <?= DOMAIN ?> and You
				...
				<p>If You have any questions or comments about these Terms or the Service, please
				   contact us at <a href = "mailto:support@<?= DOMAIN ?>.com">support@<?= DOMAIN ?>.com</a>
				</p>
